                        <div class="row">
                            <div class="col-md-4 text-center">
                                <div class="avatar-upload">
                                    {{"@"}}include('brackets/admin-ui::admin.includes.avatar-uploader', [
                                        'mediaCollection' => app(\Aligilani\AdminAuth\Models\AdminUser::class)->getMediaCollection('avatar'),
                            ...
                            <div class="col-md-8">
                                ...
                                @elseif($col['name'] == 'language')<div class="form-group row align-items-center" :class="{'has-danger': errors.has('{{ $col['name'] }}'), 'has-success': fields.{{ $col['name'] }} && fields.{{ $col['name'] }}.valid }">
                                    <label for="{{ $col['name'] }}" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-3'">{{'{{'}} trans('admin.{{ $modelLangFormat }}.columns.{{ $col['name'] }}') }}</label>
                                    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-7'">
                                        <multiselect v-model="form.{{ $col['name'] }}" placeholder="@{{ trans('brackets/admin-ui::admin.forms.select_an_option') }}" :options="@{{ $locales->toJson() }}" open-direction="bottom"></multiselect>
