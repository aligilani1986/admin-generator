    ...
    protected function buildView() {
        ...
        return view('brackets/admin-generator::' . $this->view, [
            ...
            'columns'                   => $this->readColumnsFromTable($this->tableName)->reject(function ($column) {
                return ('text' == $column['type'] || in_array($column['name'], ["password", "remember_token", "slug", "created_at", "updated_at", "deleted_at"])
                    || ('json' == $column['type'] && in_array($column['name'], ["perex", "text", "body"]))
                );
            })->map(function ($col) {
                ...
                if ('date' == $col['type'] || 'time' == $col['type'] || 'datetime' == $col['type']) {
                    $filters->push($col['type']);
                }

                if ('boolean' == $col['type'] && ('enabled' == $col['name'] || 'activated' == $col['name'] || 'is_published' == $col['name'])) {
                    $col['switch'] = true;
                }
